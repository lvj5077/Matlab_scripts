...
for i = 1:1:length(traj)
    ...
    cx = intrinsics(i,5);
    cy = intrinsics(i,6);
    color_pth = path+"/color/"+num2str(i)+".png";
    depth_pth = path+"/depth/"+num2str(i)+".png";
    imcolor = imread(color_pth);
    imdepth = imread(depth_pth);
    imcolor3col1 = reshape(imcolor,256*192,3);
...
%     ptCloud2 = pointCloud(xyzPoints, 'Color', imcolor3col1); 
    ...
    T1(1:3,1:3) = quat2rotm([traj(i,8),traj(i,5:7)]);
